...
class MY_Controller extends CI_Controller
{
    ...
    public function form_validation($type = '')
    {
        ...
        } else if ($type == 'kategori-kebaikan') {
            // Aturan validasi untuk kategori kebaikan
            $this->form_validation->set_rules(
                'name',
                'Nama Kebaikan',
                'required',
                array(
                    'required' => 'Nama Kebaikan Wajib Diisi',
                )
            );
            if ($this->form_validation->run()) {
                return true;
    ...
    public function session($level = '')
    {
        ...
        if ($level == 'Guru') {
            if (!($akses_level == "Admin" || $akses_level == "Guru" || $akses_level == "Kepala Sekolah")) {
                $this->session->set_flashdata('result', ['alert' => 'danger', 'title' => 'Ups !!', 'msg' => 'Akses Dilarang !!']);
                redirect('auth/login');
            }
        }
