...
class Pelanggaran_model extends CI_Model{
    ...
    function DataPelanggaran($limit, $start, $value = null){  
        ...
        $this->db->select($this->table . '.*, ' . $this->table_join5 . '.id as violation_id, siswa.std_name, kelas.class_name, guru.teacher_name, wali.parent_name, kelas.wali_name, tipe_pelanggaran.violation_name');
        $this->db->join($this->table_join5, $this->join5);       
        ...
        $this->db->join('tb_tipe_pelanggaran tipe_pelanggaran', 'tipe_pelanggaran.id = ' . $this->table . '.type_id', 'left');
